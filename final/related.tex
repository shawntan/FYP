...
% Jesse: You cant start a chapter with a sentence that refers to a NP from a
% different chapter
In order to devise such an algorithm, we need to predict how often any user may 
update a page. 
% Jesse: "user may update a page"? That may make sense to you, but it makes no
% sense using the more obvious meaning of the word "update"
...
database fresh.
% Jesse: this introduction paragraph has made no effort to explain why the
% related work consists of two subsections: referesh policies and thread
% content analysis. 
...
forum site.
% Jesse: After you generate the PDF, remember to look out for dangling
% paragraphs (paragraphs that overflow to the next page with only 1-5 words)
% and dangling sentences (sentences that finish a paragraph with only 1-5
% words forming the last line of the paragraph)
...
% Jesse: I thought this subsection is discussing "refresh policies", what does
% leveraging content have to do with "refresh policies"? What can you comment
% about these works regarding their "refresh policies" and how does your
% method fit into these existing works with regards to "refresh policies" and
% why is your method better / different.
...
% Jesse: This subsection seems to end with a cliff hanger. You've mentioned
% two works. One is a method to link posts together into a series of chains.
% You mention that you are not using this method. The second work proposes a
% method using a two level classification using labelled LDA. And youve made
% no comments on this work.
