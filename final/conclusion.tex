...
% Jesse: Remember to transition into the contributions subsection and make
% sure you dont downplay the significance of your contributions
...
% Jesse: You've spent quite a significant amount of time on metrics. If this
% is one of your significant contributions (and it is!), you can consider
% adding a subsection on metrics in the related work if there is time. I
% haven't read your metrics chapter yet so be careful of duplicating content
% if you do so.
\item Proposed 3 different methods that can be employed for making revisit time 
	...
	crawler.
% Jesse: no comment on the performance of these methods? What is the
% contribution? That you proposed THREE, that you proposed three that has
% great PERFORMANCE, or that you proposed three that can be used to MAKE
% REVISIT TIME ESTIMATIONS? The way the sentence sounds now, your
% contribution is that you made 3 methods instead of 1 or 2.
\item Showed that using content as part of the feature set contributes to a 
	better model for estimating posting times.
% Jesse: again, is the results statistically significant. If so, mention it
% here.
\end{enumerate}
